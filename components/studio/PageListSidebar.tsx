    ...
    >
      ...
      <div
        ...
        className={twMerge(
          ...
          isSelected
            ? 'ring-2 ring-[var(--studio-select)] shadow-[0_0_0_1px_rgba(124,92,255,0.35),0_8px_20px_-6px_rgba(124,92,255,0.45)]'
      ...
      <div className="mt-2 flex items-center gap-1 px-0.5">
        <span
          className={twMerge(
            ...
            isSelected ? 'text-[var(--studio-select)]' : 'text-neutral-400'
          )}
        >
          {page.page_number}
        </span>
        ...
        {/* Insert a page here. `addPage` only appends, so adding a page after
            page 3 of 20 meant appending and dragging it back seventeen places. */}
        <button
          type="button"
          onClick={(e) => {
            e.stopPropagation()
...
export function PageListSidebar({ onPageSelected }: PageListSidebarProps = {}) {
  const { book, currentPageIndex, setCurrentPageIndex, addPage, insertPage, removePage, reorderPages, selectBlock, selectHotspot, selectedBlockId, selectedHotspotId } =
    useEditorStore()
                ...
                <div className="grid grid-cols-2 gap-x-3 gap-y-4 sm:grid-cols-3 lg:grid-cols-2">
                  {pages.map((page, index) => (
                    <SortablePageItem
                      ...
                      onInsertAfter={() => {
                        insertPage(index)
                        toast.success(`Page inserted after ${page.page_number}`)
                      }}
            ...
            <div className="space-y-1">
              ...
              {currentPage.blocks.map((b) => (
                <button
                  ...
                  className={twMerge(
                    ...
                    selectedBlockId === b.id ? "bg-[var(--studio-select)]/20 text-[var(--studio-select)]" : "hover:bg-neutral-800 text-neutral-300"
                  )}
                >
              ...
              {currentPage.hotspots.map((h) => (
                <button
                  ...
                  className={twMerge(
                    "flex items-center gap-2 w-full p-2 text-left text-xs rounded transition-colors",
                    selectedHotspotId === h.id ? "bg-[var(--studio-select)]/20 text-[var(--studio-select)]" : "hover:bg-neutral-800 text-neutral-300"
                  )}
                >
