// Generates a branded, looping demo GIF for Riffle using sharp.
// Run: node scripts/build-demo-gif.mjs  → public/riffle-demo.gif
...
function frameCta(lp) {
  ...
  return `
    ...
    <text x="${W / 2}" y="${384 + rise(0.4)}" font-family="${SANS}" font-size="14" fill="rgba(255,255,255,0.5)"
      text-anchor="middle" opacity="${op(0.4)}">riffle.app</text>`
}
...
async function main() {
  ...
  await sharp(frames, { join: { across: 1, animated: true } })
    ...
    .toFile('public/riffle-demo.gif')
  console.log(`✓ public/riffle-demo.gif  (${N} frames, ${W}×${H})`)
}
