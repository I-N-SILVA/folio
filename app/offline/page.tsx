    ...
    <main className="qlico-grain flex min-h-screen items-center justify-center bg-[var(--background)] p-6 text-[var(--qlico-ink)]">
      <div className="max-w-md rounded-[2.25rem] border border-[var(--qlico-border)] bg-[var(--qlico-paper)]/85 p-10 text-center shadow-[var(--qlico-shadow)] backdrop-blur">
        ...
        <p className="mt-3 text-sm leading-6 text-[var(--qlico-muted)]">
          QLICO needs a connection to load this page. Reconnect and try again — your work is saved.
        </p>
        <Link
          href="/dashboard"
          className="mt-7 inline-block rounded-full bg-[var(--accent)] px-6 py-3 text-[15px] font-semibold text-[var(--accent-contrast)] transition hover:bg-[var(--accent-hover)]"
        >
          Retry
        </Link>
      </div>
